...
export default function NoteCard({ id, title, subject, date, preview, color = "primary" }: NoteCardProps) {
  const colorClasses = {
  ...
  return (
    <Card className="overflow-hidden hover-elevate transition-all duration-300 hover:shadow-xl" data-testid={`card-note-${id}`}>
      <div className={`p-6 space-y-4 border-l-4 ${colorClass}`}>
        ...
        <Link href={`/notes/${id}`} data-testid={`link-view-note-${id}`}>
